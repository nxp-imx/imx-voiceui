...
uint32_t rdsp_plugin_malloc_GetAllocatedBytes(void) {
	return ext_mem_bytes_used;
}

void* rdsp_plugin_malloc(size_t Asize, MemAlign_t align) {
#ifdef RDSP_ENABLE_MEMCHECK
	void* p = memcheck_malloc_align(Asize, align, NULL, 0, NULL);
	return p;
#else
	void* pRet = NULL;
	uint32_t aligned_size = (Asize + align - 1) & ~(align - 1);
	...
	if (ext_mem_bytes_left >= aligned_size) {
		tmp = (uintptr_t)ext_mem_nextptr;
		pRet = (void*)((tmp + (uintptr_t)(align - 1)) & ~(uintptr_t)(align - 1));

		ext_mem_nextptr = (void*)((uintptr_t)pRet + aligned_size);
		ext_mem_bytes_left -= aligned_size;
		ext_mem_bytes_used += aligned_size;

		// Check for alignment
		if ((uint64_t)pRet % align)
			failed = 1;
	}
	else
		failed = 1;
...
uint32_t rdsp_plugin_scratch_GetAllocatedBytes(void) {
...
void* rdsp_plugin_scratch_malloc(size_t Asize, MemAlign_t Aalign) {
	...
	if (ext_mem_scratch_bytes_left >= aligned_size) {
		uint64_t tmp = (uint64_t)(intptr_t)ext_mem_scratch_nextptr;
		pRet = (void *) (intptr_t)((tmp + Aalign - 1) & ~(uint64_t)(Aalign - 1));
		ext_mem_scratch_nextptr = (void*)(intptr_t)((uint64_t)((intptr_t)pRet) + aligned_size);
...
#ifdef RDSP_ENABLE_PRINTF
		printf("Not enough memory available! Requested %z scratch bytes but only %z scratch bytes available.\n", Asize, ext_mem_scratch_bytes_left);
#endif
	}
#endif // RDSP_MEMORY_UTILS_USES_STDLIB==1
	return pRet;
...
void rdsp_plugin_memset_float(rdsp_float* Ay, rdsp_float Aval, size_t Alength) {
	for (size_t i = 0; i < Alength; i++)
		Ay[i] = Aval;
}

void rdsp_plugin_memset_complex(rdsp_complex* Ay, rdsp_complex Aval, size_t Alength) {
	for (size_t i = 0; i < Alength; i++) {
...
void rdsp_plugin_memcpy_32b(void* Adest, const void* Asrc, size_t Asize) {
	uint32_t* pdest32 = (uint32_t*)Adest;
	uint32_t* psrc32 = (uint32_t*)Asrc;
	for (size_t i = 0; i < Asize; i++)
		pdest32[i] = psrc32[i];
}

void rdsp_plugin_memcpy_64b(void* Adest, const void* Asrc, size_t Asize) {
	uint64_t* pdest64 = (uint64_t*)Adest;
...
void rdsp_plugin_memcpy_float(rdsp_float* Adest, rdsp_float* Asrc, size_t Alength) {
	memcpy(Adest, Asrc, Alength * sizeof(rdsp_float));
}

void rdsp_plugin_memcpy_complex(rdsp_complex* Adest, rdsp_complex* Asrc, size_t Alength) {
	memcpy(Adest, Asrc, Alength * sizeof(rdsp_complex));
}
...
uint32_t rdsp_plugin_memcompare(void* Ax, void* Ay, size_t Asize) {
#if RDSP_MEMORY_UTILS_USES_STRING==1
